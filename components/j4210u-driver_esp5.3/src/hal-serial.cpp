...
uart_port_t platform_open(const char *reader_identifier, int baudrate)
{
...
int platform_close(uart_port_t h)
{
    uart_driver_delete(h);
    return true;
}

int platform_write(uart_port_t h,
...
int platform_read(uart_port_t h,
                  unsigned char *message_buffer,
                  unsigned short buffer_size,
