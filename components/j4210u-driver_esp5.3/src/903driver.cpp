...
namespace mu903
{
    ...
    {
        ReaderInfo old;
        bool status = mu903::GetSettings(&old);
        if (!status)
        {
    ...
    {
        scan_.clear();
        bool success = mu903::InventoryNB(scan_, filter);
        if (!success)
            return 0;
    ...
    bool SetPassword(unsigned char *epc, unsigned char epcLen, unsigned char *pass, unsigned char size)
    {
        ...
        return mu903::Write((unsigned char *)epc, epcLen, pass, size / 2, 2, MEM_PASSWORD);
    }

    bool SetKillPassword(unsigned char *epc, unsigned char epcLen, unsigned char *pass, unsigned char size)
    {
        return mu903::Write((unsigned char *)epc, epcLen, pass, size / 2, 0, MEM_PASSWORD);
    }
    ...
    {
        bool b = mu903::WriteWord(epc, epclen, data, windex, MEM_USER);
        if (!b)
        {
            unsigned char data2[2] = {0, 0};
            // printarr(epc,epcLen);
    ...
    {
        bool b = mu903::WriteWord(epc, epclen, data, windex, MEM_EPC);
        if (!b)
        {
    ...
    {
        bool ok = mu903::Write(epc, epcLen, data, epcLen / 2, 0, MEM_EPC);
        if (ok)
        {
            unsigned char newepc[epcLen];
            ok = mu903::Read(data, epcLen, newepc, epcLen / 2, 0, MEM_EPC);
    ...
    {
        return mu903::ReadWord(epc, epclen, data, windex, MEM_USER);
    }

    bool ReadEpcWord2(unsigned char *epc, unsigned char epclen, unsigned char *data, int windex, int memtype)
    {
        return mu903::Read(epc, epclen, data, 1, windex, MEM_EPC);
    }
    ...
    {
        *passlen = 8;
        return mu903::Read((unsigned char *)epc, epclen, pass, 4, 0, MEM_PASSWORD);
    }
    ...
    bool GetTID(unsigned char *epc, unsigned char epclen, unsigned char *tid, unsigned char *tidlen)
    {
        *tidlen = epclen;
        error_ = "";
        bool success = mu903::Read(epc, epclen, tid, epclen / 2, 0, MEM_TID);
        if (!success)
        {
