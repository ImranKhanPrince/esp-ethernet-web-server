...
namespace mu910
{
    ...
    bool SetSettings(ReaderInfo *ri)
    {
        ...
        bool status = mu910::GetSettings(&old);
        if (!status)
        {
        ...
        { // +7 = power has some offset 26-33 conversion
    ...
    static bool setInventoryParameters(unsigned char acsAddress, unsigned char acsLen, unsigned char memtype)
    {
        ...
        cmd.push_back(mu910::getBaudIndex(baud_)); // Baudrate* BUG: [changing baudrate causes fail to read then prev raw settings stays and that causes to change the baud after reconnected
    ...
    {
        scan_.clear();
        bool success = mu910::InventoryNB(scan_, filter);
        if (!success)
            return 0;
    ...
    bool SetPassword(unsigned char *epc, unsigned char epcLen, unsigned char *pass, unsigned char size)
    {
        ...
        return mu910::Write((unsigned char *)epc, epcLen, pass, size / 2, 2, MEM_PASSWORD);
    }

    bool SetKillPassword(unsigned char *epc, unsigned char epcLen, unsigned char *pass, unsigned char size)
    {
        return mu910::Write((unsigned char *)epc, epcLen, pass, size / 2, 0, MEM_PASSWORD);
    }
    ...
    bool GetTID(unsigned char *epc, unsigned char epclen, unsigned char *tid, unsigned char *tidlen)
    {
        *tidlen = epclen;
        error_ = "";
        bool success = mu910::Read(epc, epclen, tid, epclen / 2, 0, MEM_TID);
        if (!success)
        {
...
//    #if defined(__APPLE__) || defined(ARM)
    ...
    {
        return mu910::Read(epc, epclen, data, 1, windex, memtype); // only 1 word
    }

    bool ReadMemWord(unsigned char *epc, unsigned char epclen, unsigned char *data, unsigned char windex)
    {
        return mu910::ReadWord(epc, epclen, data, windex, MEM_USER);
    }
    ...
    {
        bool b = mu910::Write(epc, epclen, data, 1, windex, MEM_USER);
        if (b)
        {
            unsigned char data2[2] = {0, 0};
            // printarr(epc,epcLen);
            // printarr(epc2,epcLen);
            bool b = mu910::ReadWord(epc, epclen, data2, windex, MEM_USER);
            // printarr(data,2);
    ...
    {
        bool b = mu910::WriteWord(epc, epclen, data, windex, MEM_EPC);
        if (!b)
        {
    ...
    {
        bool ok = mu910::Write(epc, epcLen, data, epcLen / 2, 0, MEM_EPC);
        if (ok)
        {
            unsigned char newepc[epcLen];
            ok = mu910::Read(data, epcLen, newepc, epcLen / 2, 0, MEM_EPC);
    ...
    {
        unsigned char cmd[] = {0xCF, 0x00, 0x00, 0x81, 0x00, 0x00, 0x00};
        ...
        {
            emptyserial();
            ok = transfer(cmd, sizeof(cmd), response, sizeof(response), 20, 10);
        } while (retries-- && !ok);
        if (!ok || response[5] != 0x00)
        {
            error_ = "Failed to read GPI";
            return -1;
        }

        if (gpiNumber == 0x01)
        {
            gpiStatus = response[6] & 0x01;
        }
        else
        {
            gpiStatus = (response[6] & 0x02) >> 1;
        }
        if (Debug_)
        {
            printf("GPI %02X = %d\n", gpiNumber, gpiStatus);
    ...
    static bool setGPO1(uint8_t level)
    {
        // CF FF 00 77 02 01 00 crch crcl
        level = level + 1; // 1= low, 2 = high

        unsigned char cmd[] = {0xCF, 0xFF, 0x00, 0x77, 0x02, level, 0x00, 0, 0};
        ...
        {
            emptyserial();
            ok = transfer(cmd, sizeof(cmd), response, sizeof(response), 20, 20);
        } while (retries-- && !ok);
        if (!ok || response[5] != 0x00)
        {
            error_ = "Failed to set GPO 1";
            return 0;
        }
    ...
    static bool setGPO2(uint8_t level)
    {
        // CF FF 00 77 02 01 00 crch crcl
        level = level + 1; // 1= low, 2 = high
        // printf("level gpo2: %02X\n", level);

        unsigned char cmd[] = {0xCF, 0xFF, 0x00, 0x78, 0x02, level, 0x00, 0, 0};
        unsigned char response[10];
        int retries = 2;
        bool ok;
        do
    ...
    bool SetGPO(unsigned char gpiNumber)
    {
        bool status = 0x00;
        ...
        {
        case 0x00:
            status |= setGPO2(0); // sets if one or 0
            status &= setGPO1(0); // tries to reset if both 1 then 1 or 0
            break;
        case 0x01:
            status |= setGPO2(0);
            status &= setGPO1(1);
